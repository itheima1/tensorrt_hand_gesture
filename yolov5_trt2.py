...
class YoLov5TRT(object):
    ...
    def __init__(self, engine_file_path):
        ...
        for binding in trt_yolo.engine:
            size = trt.volume(trt_yolo.engine.get_binding_shape(binding)) * trt_yolo.engine.max_batch_size
            dtype = trt.nptype(trt_yolo.engine.get_binding_dtype(binding))
            # Allocate host and device buffers
            # host_mem = cuda.pagelocked_empty(size, dtype)
            # cuda_mem = cuda.mem_alloc(host_mem.nbytes)
            print(size)
            io_info = trt_yolo.get_io_info(i2shape)

            cuda_mem = trt_yolo.allocate_io_buffers(size, True)
            print(cuda_mem,size)
            # Append the device buffer to device bindings.
            bindings.append(int(cuda_mem))

            # Append to the appropriate list.
            if trt_yolo.engine.binding_is_input(binding):
