...
def APIToModel_Hand(maxBatchSize):
    ...
    engine = createLenetEngine(maxBatchSize,WEIGHT_PATH,INPUT_H,INPUT_W,OUTPUT_SIZE,builder,config,trt.float32)
    assert engine
...
def APIToModel_Gesture(maxBatchSize):
    builder = trt.Builder(TRT_LOGGER)
    config = builder.create_builder_config()
    engine = createLenetEngine(maxBatchSize,"./resnet50-gesture.wts",224,224,14,builder,config,trt.float32)
    assert engine

    with open("resnet-gesture.engine","wb") as f:
        f.write(engine.serialize())
...
if __name__ == '__main__':
    ...
    if args.s:
        ...
        # 识别人手的21个关键点
        trt_lite = TrtLite(engine_file_path=ENGINE_PATH)
        trt_lite.print_info()
        ...
        i2shape = 1#{0: (BATCH_SIZE * 3 * INPUT_H * INPUT_W,)}
        io_info = trt_lite.get_io_info(i2shape)
        print(io_info)
        d_buffers = trt_lite.allocate_io_buffers(i2shape, True)
        print(io_info[1][2])
        output_data_trt = np.zeros(io_info[1][2], dtype=np.float32)
        #
        # d_buffers[0] = img_.ravel()
        # trt_lite.execute([t.data_ptr() for t in d_buffers], i2shape)
        #
        #
        ...
        # assert runtime
        #
        # with open("./resnet-gesture.engine", "rb") as f:
