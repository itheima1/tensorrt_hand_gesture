...
def doInference(trt_engine,data):
    i2shape = 1
    ...
    # 进行推理
    trt_engine.execute([t.data_ptr() for t in d_buffers], i2shape)
...
if __name__ == '__main__':
    ...
    if args.s:
        ...
        # io_info = trt_lite21.get_io_info(i2shape)
        # print(io_info)
        # d_buffers = trt_lite21.allocate_io_buffers(i2shape, True)
        # print(io_info[1][2])
        # output_data_trt = np.zeros(io_info[1][2], dtype=np.float32)
        #
        #
        #
        # d_buffers[0] = img_.ravel().cuda()
        #
        # bindings = [t.data_ptr() for t in d_buffers]
        # print(bindings)
        #
        # trt_lite21.execute([t.data_ptr() for t in d_buffers], i2shape)
        #
        # #
        # output_data_trt = d_buffers[1].clone().cpu().detach().numpy()
        #
        ...
        host_out = doInference(trt_lite21,img_.ravel())
        # 绘制人手
        drawhand(img,host_out)


        # # 识别人的手势
        host_out = doInference(trt_lite_gesture,img_.ravel())
